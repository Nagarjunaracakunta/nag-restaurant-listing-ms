pipeline {
  ...
  steps {
    sh 'mvn clean org.jacoco:jacoco-maven-plugin:prepare-agent install sonar:sonar -Dsonar.host.url=http://3.10.4.236:9000/ -Dsonar.login=squ_96e3e525784df444d9d2ea5c9ba15cf7cbbca36c'
  }
}
   ...
   stage('Check code coverage') {
    ...
    stage('Update Image Tag in GitOps') {
      steps {
         checkout scmGit(branches: [[name: '*/master']], extensions: [], userRemoteConfigs: [[ credentialsId: 'git-ssh', url: 'git@github.com:Nagarjunaracakunta/deployment.git']])
        script {
       sh '''
